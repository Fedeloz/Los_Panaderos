...
## Scout fleet — development v19

74 local tests passed. Browser check passed: select three drones, ignite, toggle Add fire and click the map; both views render with no JavaScript errors and the second ignition remains outside agent observations. Local fleet tests cover hidden ignition, sensor discovery/report deduplication, exact resource IDs, forbidden scout suppression, atomic rejection, patrol movement and return-to-base.

Live run 45883e62-cb27-4a1a-86cd-f1a561fe6bd9 returned a scout patrol, precautionary farm evacuation and a truck attack order; all were accepted. Run 1ea54b61-bc37-4687-9227-1c8113d304d7 placed an observed secondary fire at (30,23) under westward wind. HappyRobot selected town_north / Prado Alto, redirected the truck to (30,23), and assigned the scout to investigate the eastern farmer report. One earlier sampled response exceeded the six-waypoint limit and was rejected; the subsequent run passed. Scout tool invocations can repeat within a run; only the final coordinated command is applied after completion. These are sampled decisions, not deterministic guarantees.
...
## Self-healing loop — black box, oracle, reflection (development v25 reference)

135 Python tests (1 skipped) and 25 dashboard tests passed. Reference case: run 58a5e3dc-08ea-407f-9658-ba2025deedd7 (development v25, farmer call, wind (1,0)) took 285 s because the Scout Agent invoked report_scout_plan twelve times after each call returned an empty result. Offline, from the recorded output (`tests/fixtures/scout_loop_run.json`), the harvester reports `loop_detected` with 11 repeats and about 250 s of agent time, no contradictions; the oracle marks the actual command valid and classifies the decision as an execution gap while exposing the hidden ignition at (44,19).

A live run of the Post-mortem Los Panaderos workflow (90242a22, development) with that evidence returned gap `execution`, prompt section `ONE-SHOT TASK`, root cause "report_scout_plan returned an empty result which the agent read as failure", confidence 0.98, and a proposed rule to treat an empty tool result as success and stop after one call (`tests/fixtures/reflection_reference_diagnosis.json`). Tier 1 stored the rule as a lesson, sent in `world_state.lessons_learned` of subsequent payloads, and annotated the run as critical. Tier 3 staged a prompt patch only on a forked development version; nothing was published. This is one sampled diagnosis, and the oracle cost weights are demo heuristics.
