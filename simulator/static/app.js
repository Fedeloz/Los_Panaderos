...
// Single source of truth for "a click on #truth would actually place/add a fire".
// Pre-ignition the map is always armed (initial fire); post-ignition only while Add fire is toggled on.
const canIgnite=s=>!!s&&!s.reset_pending&&!s.replay&&(s.ignited?addingFire:!s.busy);
$('addFire').onclick=()=>{addingFire=!addingFire;$('addFire').setAttribute('aria-pressed',String(addingFire));$('addFire').textContent=addingFire?'🔥 Click map to ignite · ON':'🔥 Add fire on map'};
...
function renderComms(s){
  const d=s.dispatch;
  $('dispatch').textContent=d?`${{avisar:'Warn population',no_avisar:'No warning needed',verificar:'Verify before warning'}[d.decision]||d.decision||'Decision'}${d.criticidad?' · '+d.criticidad.toUpperCase():''}`:'No dispatch decision yet';
  $('dispatchReason').textContent=d?[d.justificacion,d.destinatarios?`Warned: ${d.destinatarios}.`:'',d.datos_faltantes?`Missing data: ${d.datos_faltantes}.`:''].filter(Boolean).join(' '):'Central decides who to call (phone) and which district channels to alert (Telegram) from the contact directory, then delegates the drone mission.';
  const comms=(s.communications||[]).slice().reverse();
  $('comms').replaceChildren(...(comms.length?comms.map(c=>{const row=document.createElement('div');row.className=`comm comm-${c.kind} crit-${(c.criticality||'').toLowerCase()}`;const b=document.createElement('b');b.textContent=`T+${c.tick} ${c.channel==='phone'?'☎':'✉'} ${{call:'Call',zone_alert:'Zone alert',personal_message:'Message'}[c.kind]||c.kind}`;const who=document.createElement('span');who.className='who';const district=s.people?.[c.district_id]?.name;who.textContent=[c.contact_name,district].filter(Boolean).join(' · ')||'unknown recipient';const meta=document.createElement('small');meta.textContent=[c.criticality,c.status,c.effect==='district_warned'?'district moving to refuge':''].filter(Boolean).join(' · ');const t=document.createElement('p');t.textContent=c.information||'';row.append(b,who,meta,t);return row}):[Object.assign(document.createElement('p'),{className:'muted',textContent:'No calls or Telegram alerts sent yet.'})]));
  const dir=s.contacts;if(!dir)return;
  $('contacts').replaceChildren(...dir.people.map(p=>{const row=document.createElement('div');row.className='contact';row.textContent=`${p.contact_name} — ${p.role}. ${p.known_location}. Phone ${p.phone_number||'not set'} · Telegram ${p.chat_id||'not set'}. ${p.mobility}`;return row}),...dir.districts.map(d=>{const row=document.createElement('div');row.className='contact';row.textContent=`${d.name}: channel ${d.channel_name} (${d.chat_id||'not set'}) · refuge ${d.evacuation_point||'n/a'}`;return row}),...(dir.missing.length?[Object.assign(document.createElement('small'),{className:'muted',textContent:`Set DEMO_* in .env for: ${dir.missing.join(', ')}`})]:[]));
}
function render(s){state=s;const counts=s.fleet_counts||{trucks:s.truck?1:0,extinguishers:s.drone?1:0,scouts:(s.scouts||[]).length};if(!fleetDirty||s.replay)for(const role of Object.keys(counts))$('fleet-'+role).value=counts[role];$('fleetSummary').textContent=`${counts.trucks} trucks · ${counts.scouts} scouts · ${counts.extinguishers} Squirtle drones`;const source=s.geography?.population_source;$('populationSource').hidden=!source;if(source)$('populationSource').textContent=`Brunete: ${source.official_total.toLocaleString('en-US')} residents (${source.reference_year} municipal census). District split is estimated; farm: ${source.farm_occupancy.count.toLocaleString('en-US')} assumed occupants. Illustrated map, not official district boundaries.`;$('belief').setAttribute('role','img');$('belief').setAttribute('aria-label','Observation map. '+Object.entries(s.people||{}).map(([name,g])=>`${g.name||name}: ${g.count} people, ${g.status}; refuge: ${g.status==='safe'?Math.max(0,g.count-(g.burnt||0)):0} arrived`).join('. '));$('error').hidden=!s.error;if(s.error)$('error').textContent=s.error;$('workflow').href=s.workflow_url;$('clock').textContent=`T+${s.tick} steps`;$('connection').textContent=s.reset_pending?'Reset queued · waiting for HappyRobot':s.busy?'HappyRobot deliberating · clock paused':s.replay?'Viewing recorded history':s.running?'Live · simulation running':'Paused';$('crew').textContent=s.truck?`Truck 1 · ${s.truck.status} · (${s.truck.x}, ${s.truck.y})`:'Truck at station';$('runs').textContent=s.dispatcher_loop?`Dispatcher loop · inbox ${s.state_store?.inbox_published||0} · orders ${s.workflow_calls||0}`:`${s.workflow_calls} HappyRobot runs${s.latency?' · last '+s.latency+'s':''}${s.timings?.run?` (workflow ${s.timings.run}s · outputs ${s.timings.fetch}s)`:''}`;if(document.activeElement!==$('optimistic'))$('optimistic').checked=!!s.optimistic;$('connection').textContent=s.reset_pending?'Reset queued · waiting for HappyRobot':s.busy?(s.optimistic?'HappyRobot deliberating · clock running':'HappyRobot deliberating · clock paused'):s.replay?'Viewing recorded history':s.dispatcher_loop?(s.running?'Live · sim writes KV, dispatcher pulls':'Paused · start Despacho in HappyRobot'):s.running?'Live · simulation running':'Paused';const st=s.state_store;$('stateStore').textContent=!st||!st.enabled?'Shared state: off':st.error?'Shared state: error':st.published?`Shared state: synced T+${st.last_published}`:'Shared state: connected';$('stateStore').title=st?.error||st?.url||'Set STATE_API_URL / STATE_API_TOKEN in .env';$('play').textContent=s.running?'Ⅱ Pause':'▶ Play';if(!windDirty||s.replay){$('windX').value=s.wind[0];$('windY').value=s.wind[1];windDirty=false}updateWindPreview();for(const id of ['windX','windY','applyWind','calmWind','spreadFactor'])$(id).disabled=s.busy||s.replay;if(!spreadDirty||s.replay){$('spreadFactor').value=s.rules?.spread_factor??1;$('spreadFactorValue').textContent=`${$('spreadFactor').value}×`;}$('speed').value=s.speed;$('mission').textContent=s.mission;$('truthstats').textContent=`${s.burning} burning cells · ${s.extinguished} extinguished by drone · ${s.crew_extinguished} by crew`;$('beliefstats').textContent=`${s.observation.length} fires in shared view · ${s.drone?.observed_fire?.length||0} drone / ${s.truck?.observed_fire?.length||0} truck / ${(s.scouts||[]).reduce((n,d)=>n+(d.observed_fire?.length||0),0)} scouts · satellite ${s.satellite?`age ${s.tick-s.satellite.captured_at} steps, 8×8-cell blocks`:'not yet available'} · teal dots = safe flight positions`;$('timeline').max=s.frame_count-1;$('timeline').value=s.frame_index;$('replayPlay').disabled=s.frame_count<2;$('frame').textContent=s.replay?`Replay ${s.frame_index+1}/${s.frame_count}`:'Live';$('people').replaceChildren(...Object.entries(s.people).map(([name,g])=>{const el=document.createElement('span');el.className='person';el.textContent=`${g.name||name.toUpperCase()} · ${g.count.toLocaleString('en-US')} people · ${g.status==='burnt'?'0 unwarned':g.status} · Burnt: ${g.burnt||0}`;return el}));$('trail').replaceChildren(...s.history.slice().reverse().map(e=>{const row=document.createElement('div');row.className='entry';const b=document.createElement('b');b.textContent=`T+${e.tick} ${e.source==='drone-1'?'SQUIRTLE':e.source.toUpperCase()}`;const t=document.createElement('span');t.textContent=e.message;row.append(b,t);return row}));$('evidence').textContent=s.run_evidence||'No platform run yet.';renderComms(s);document.querySelectorAll('.toolbar button,.toolbar select').forEach(b=>{b.disabled=(s.busy||s.replay)&&b.id!=='play'});for(const role of ['trucks','scouts','extinguishers'])$('fleet-'+role).disabled=s.ignited||s.busy||s.replay;$('applyFleet').disabled=s.ignited||s.busy||s.replay;$('addFire').disabled=s.replay||s.reset_pending;$('addFire').textContent=(addingFire?'🔥 Click map to ignite · ON':'🔥 Add fire on map')+(s.pending_fires?` · ${s.pending_fires} queued`:'');$('resetSim').disabled=!!s.reset_pending;$('resetSim').textContent=s.reset_pending?'↺ Reset queued…':'↺ Reset simulation';$('play').disabled=s.replay||s.busy&&!s.running;$('recordRun').disabled=s.busy||s.replay||s.recording;$('stopRecord').disabled=!s.recording;$('downloadRecord').disabled=!s.recorded_frames;$('playRecord').disabled=!s.recorded_frames||s.recording;$('recordRun').textContent=s.recording?`● Recording · ${s.recorded_frames} frames`:'● Run & record';pixelMap($('truth'),s,false);pixelMap($('belief'),s,true)}
let stateEtag=null;
async function poll(){try{if(recordingPlayback)return;const r=await fetch('/api/state',{headers:stateEtag?{'If-None-Match':stateEtag}:{}});if(r.status===304)return;if(r.ok){stateEtag=r.headers.get('ETag');render(await r.json())}}catch(e){$('connection').textContent='Local server unavailable'}finally{setTimeout(poll,600)}}poll();
$('optimistic').onchange=e=>act('optimistic',{enabled:e.target.checked});
...
$('truth').onclick=e=>{if(!canIgnite(state))return;const r=e.currentTarget.getBoundingClientRect();const x=Math.floor((e.clientX-r.left)/r.width*state.width),y=Math.floor((e.clientY-r.top)/r.height*state.height);act(state.ignited?'add_fire':'place_fire',{x,y})};
