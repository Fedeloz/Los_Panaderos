...
window.AerialView = (() => {
  ...
  function vehicle(c,v,p,drone,time){
    ...
    c.restore();text(c,x-18,y+(drone?-18-(v.role==='scout'?18*Number(v.drone_id.split('-')[1]):0):27),drone?(v.role==='scout'?v.drone_id.toUpperCase():'EXTINGUISHER'):'TRUCK 01');
...
$('truth').onclick=e=>{if(!state||state.busy||state.replay||(state.ignited&&!addingFire))return;const r=e.currentTarget.getBoundingClientRect();const x=Math.floor((e.clientX-r.left)/r.width*state.width),y=Math.floor((e.clientY-r.top)/r.height*state.height);act(state.ignited?'add_fire':'place_fire',{x,y})};
