...
Suppression context: current runtime rules explicitly override older fixed-capacity examples in the published prompts. With enough targets, expected extinguishing is 0.4 cells/step for the drone and 3 for the truck; maxima are 1 and 5. Failures leave fire burning, and each vehicle attempts a cell at most once per step. Jet visual effects show attempts; extinguished counters count successes only.

Fire spread control: choose 0.25×–4× in the wind panel (default 1×). This divides wind-dependent ignition attempt intervals by the factor, rounded to whole steps with a one-step minimum. It leaves the 50% ignition chance, vehicle speed and suppression unchanged. Agents receive the factor and resulting intervals; recording frames preserve it. Reset restores 1×.
